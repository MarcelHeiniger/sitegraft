...
for a backup that wasn't actually taken.
...
subfolder, complete with its own generated `restore.sh` and its own manifest, so
even a restore stays reversible. Asks for confirmation unless `--yes` is passed.
`<run-id>` is the directory name `scan` created (printed at the end of every phase,
and listable under `SITEGRAFT_STATE_DIR/<profile>-*`).
...
tar, gzip/gunzip, wc, and on a containerized target find, sort, comm, mktemp and
xargs), so it still works even if you no longer have a sitegraft checkout on hand.

**What "restore" means, exactly.** `restore.sh` prints its own semantics when it
runs, so you never have to infer them. On every target shape it restores
`wp-content` to *exactly* what the backup contained — files added since the backup
(a grafted theme, copied plugins, new uploads) are removed, not left behind. How it
gets there depends on the target:
...
nothing at all. (A `restore.sh` generated by an older sitegraft is printed rather
than executed — it would ignore the flag and perform a real restore.) You can run
that preview yourself, too: `./run-dir/restore.sh --dry-run`.
...
| `--dry-run` | all | Print what would happen instead of doing it. `scan`/`plan`/`verify` accept it too for CLI consistency, but each is read-only (or writes only locally) already, so it's a safe no-op there — the real reads/checks always run. On `restore` it goes further and previews for real: it runs `restore.sh --dry-run`, which reads B and lists every path it would remove, writing nothing. |
